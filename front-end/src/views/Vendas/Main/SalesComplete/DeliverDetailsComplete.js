...
const DeliverDetailsComplete = ({ id }) => {
  ...
  return shipments.map((shipment, index) => {
    ...
    return (
      <div key={index} className="deliver-details border border-dark rounded p-0 mb-2 mb-xl-0 h-100">
        <div className="sales-header d-flex justify-content-between p-2 items-header">
          ...
          <DeliveryDisplay mode={shipping_abbreviation} mode_name={shipping_name} />
        </div>
        <div className="sales-body p-2">
          <div className="d-flex justify-content-between flex-column mb-2">
            ...
            <p className="mb-0 ">
              {receiver_street_name}, nº {receiver_street_number}, {receiver_city}
            </p>
            <p className="mb-0 flex-grow-1">
              CEP: <span className="font-weight-normal">{receiver_zip_code}</span>
            </p>
          </div>
          ...
          <div className="mb-2">
            <p className="mb-0 mr-auto">Custo listado: {formatMoney(list_cost)}</p>
          </div>
          <div className="d-flex justify-content-between align-items-center mb-0">
          ...
          <div className="mt-2 d-flex justify-content-between align-items-center flex-wrap">
            <p className="mb-0 salescard-body-title d-inline d-md-block d-xl-inline">Status:</p>
            <p className="mb-0 ml-2 mr-auto">{status}</p>
            {/* <Button className="btn btn-info text-white p-1 flex-grow-0 flex-grow-lg-1 ml-auto mt-0 mt-lg-1">
              {" "}
              <i className="cil-qr-code mr-1" /> Etiqueta
            </Button> */}
          </div>
        </div>
      </div>
