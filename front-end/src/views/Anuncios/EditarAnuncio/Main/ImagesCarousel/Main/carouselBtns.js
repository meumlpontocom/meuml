...
const highQualityDispatch = (dispatch, action) => {
  const highQualityLocation = "editar-para-alta-qualidade";
  const locationHrefSplit = window.location.href.split("#/")[1];
  const isHighQualityView = locationHrefSplit === highQualityLocation;
  if (isHighQualityView) dispatch(action);
};

export function AddNewPictureButton({
  items,
  accountIdFromOtherViews,
  setIsLoadingCallback,
  setNewPictureIdCallback,
}) {
  ...
  const savePicture = async ({ pictureTitle, picture }) => {
    try {
      highQualityDispatch(dispatch, toggleIsLoading());
      setIsLoadingCallback(true); // callback
      const formData = new FormData();
      formData.append("account_id", accountId || accountIdFromOtherViews);
      ...
      highQualityDispatch(dispatch, saveNewAdvertPicture(data.id));
      setNewPictureIdCallback(data.id);
      Swal.fire({
        title: "Atenção",
        text: message,
        type: status,
      ...
      highQualityDispatch(dispatch, toggleIsLoading());
      setIsLoadingCallback(false); // callback
    }
  };
...
export function DeleteCurrentPictureButton({ items, activeIndex, deleteImageCallback }) {
  const dispatch = useDispatch();

  function deleteImage() {
    deleteImageCallback(activeIndex);
    highQualityDispatch(dispatch, removeAdvertImage(activeIndex));
  }
...
export function IndexControlButtons({ items, previous, next }) {
  return (
    <Row className="justify-content-around mx-auto" style={{ width: "212px", marginBottom: "30px" }}>
