...
function AdvertPrice({
  ...
  const soldQuantityStatus = components.components.filter(
    x => x.code === "sold_quantity"
  );
  const availableQuantityStatus = components.components.filter(
    x => x.code === "available_quantity"
  );
  ...
  return (
    <Provider value={{ render: true, status, externalId}}>
      <td id="price" name="price" style={{ verticalAlign: "middle" }}>
        ...
        <SoldQuantity
          render={soldQuantityStatus[0].status}
          amount={soldQuantity}
        />
        <AvailableQuantity
          render={availableQuantityStatus[0].status}
          amount={availableQuantity}
        />
        <Status
          render={statusStatus[0].status}
          status={status}
