...
def advertising_description_replace_text_many(pool, user_id: int, filter_query: str, filter_values: dict, replace_from: str, replace_to: str, tag: str = None):
    advertising_description_replace_text_item = queue.signature('items_queue:advertising_description_replace_text_item')

    action = QueueActions()
    action.conn = get_conn()

    try:
        tool = get_tool(action, 'replace_description')
        if tool is None:
        ...
        for account_id, account in accounts.items():
            ...
            for advertising in advertisings:
                ...
                if tool['access_type'] == AccessType.free or tool['access_type'] == AccessType.subscription or (tool['access_type'] == AccessType.credits and use_credits(action, user_id, process_item_id, tool['price'])):
                    advertising_description_replace_text_item.delay(
